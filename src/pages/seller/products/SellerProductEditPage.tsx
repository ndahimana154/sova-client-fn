...
export function SellerProductEditPage() {
  ...
  useEffect(() => { if (product) setAttributes(attributeRows(product.variants)) }, [product])
  ...
  return <div className="space-y-5 p-5"><PageTitle title="Edit product" subtitle={`Update ${product.name}.`} /><Feedback error={loadError} />
    <form className="seller-card space-y-5 p-5" onSubmit={submit}><div className="grid gap-4 md:grid-cols-2">
      ...
      <Field label="Price (RWF)"><input defaultValue={product.price} min="0" name="price" required step="1" type="number" /></Field><Field label="Discount (RWF)"><input defaultValue={product.discount} min="0" name="discount" step="1" type="number" /></Field><Field label="Quantity"><input defaultValue={product.quantity} min="0" name="quantity" step="1" type="number" /></Field>
