...
export function CategoryRail({ onCategoryOpen }: { onCategoryOpen: (category: string) => void }) {
  return (
    <section className="page-container section-space" id="categories">
      ...
      <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-6">
        {categories.map((category) => (
          <button className="category-card group" key={category.name} onClick={() => onCategoryOpen(category.name)} type="button">
